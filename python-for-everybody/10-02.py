...
for line in fh :                                       # begins cycling through each line in the file
    ...
    if line.startswith('From ') :                      # looks for lines that start with 'From '
        ...
        time = newline[5]                              # turns the 6th value (time) into a new string
        ...
        counts[hour] = counts.get(hour, 0) + 1         # creates a dictionary with the hour # as the key, and the number of times it appears as the value

for k, v in sorted(counts.items()) :                   # prints out the key/value in sorted order of dictionary counts 
    print(k, v)